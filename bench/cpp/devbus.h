...
public:
	...
	// readz: Read a series of values from the same addr in memory.  This 
	// {{{
	...
	// poll: Query whether or not an interrupt has taken place
	virtual	bool	poll(void) = 0;

	// usleep: Sleep until interrupt, but sleep no longer than msec milliseconds
	virtual	void	usleep(unsigned msec) = 0;

	// wait: Sleep until an interrupt, no matter how long it takes for that
	// {{{
